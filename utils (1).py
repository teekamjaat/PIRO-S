...
async def is_subscribed(bot, user_id):
    ...
    for channel_link in AUTH_CHANNELS:
        try:
            chat = await bot.get_chat(channel_link)  # Get channel details using link
            user = await bot.get_chat_member(chat.id, user_id)

            if user.status == enums.ChatMemberStatus.BANNED:
                return False  # User is banned from a channel

        except UserNotParticipant:
            not_subscribed_channels.append(channel_link)  # Add to unsubscribed list
        except Exception:
            return False  # Handle other errors

    if not_subscribed_channels:
        return not_subscribed_channels  # Return channels user is not subscribed to
